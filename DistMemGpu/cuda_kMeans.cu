...
__global__
void HelloKernel(void)
{
    printf("HELLO FROM A CUDA KERNEL\n");
}

/**
 * wrapper:     Cuda_KMeans::do_cuda_kMeans
 * 
 * In Args:     int k, char* category1,
...
namespace Cuda_KMeans {
                      ...
                      double* sumZ)
	{
        ...
        cudaMemcpy(nPoints, cuda_nPoints, k * sizeof(int), cudaMemcpyDeviceToHost);
        cudaMemcpy(sumX,cuda_sumX,  k * sizeof(double), cudaMemcpyDeviceToHost);
        cudaMemcpy(sumY,cuda_sumY,  k * sizeof(double), cudaMemcpyDeviceToHost);
        cudaMemcpy(sumZ,cuda_sumZ, k * sizeof(double), cudaMemcpyDeviceToHost);
